...
public class JavaWarpClient implements WarpClient {
	private static JavaWarpClient _instance = null;
	...
	private JavaWarpClient.ConnectionWatchTask connectTimeoutTask;
	private JavaWarpClient.ConnectionAutoRecoveryTask connectAutoRecoveryTask;
	...
	public static JavaWarpClient getInstance() throws Exception {
		if (_instance == null) {
			throw new Exception("JavaWarpClient not initialized!");
		} else {
			return _instance;
		}
	}
	...
	void onResponse(WarpResponseMessage msg) {
		...
		switch (msg.getRequestType()) {
			...
			case 2:
				try {
					lobbyEvent = EventBuilder.buildLobbyEvent(msg);
			...
			case 6:
				try {
					roomEvent = EventBuilder.buildRoomEvent(msg);
			...
			case 17:
				try {
					allRoomsEvent = EventBuilder.buildAllRoomsEvent(msg);
			...
			case 18:
				try {
					allUsersEvent = EventBuilder.buildAllUsersEvent(msg);
			...
			case 19:
				...
				try {
					userInfoEvent = EventBuilder.buildLiveUserInfoEvent(msg);
			...
			case 20:
				try {
					liveRoomInfoEvent = EventBuilder.buildLiveRoomEvent(msg);
			...
			case 22:
				...
				try {
					customUserEvent = EventBuilder.buildLiveUserInfoEvent(msg);
			...
			case 73:
				...
				try {
					matchedRoomsEvent = EventBuilder.buildMatchedRoomsEvent(msg);
			...
			case 67:
				...
				return;
			case 68:
				MoveEvent[] moves = EventBuilder.buildMoveHistoryArray(msg);
				for (TurnBasedRoomListener listener : this.turnBasedRoomListeners) {
			...
			case 70:
				try {
					allRoomsEvent = EventBuilder.buildRoomsCountEvent(msg);
			...
			case 71:
				try {
					allUsersEvent = EventBuilder.buildUsersCountEvent(msg);
			...
			case 74:
				...
				try {
					userInfoEvents = EventBuilder.buildOnlineUserStatusEvent(msg);
			...
			case 77:
				for (TurnBasedRoomListener listener : this.turnBasedRoomListeners) {
					listener.onGetGameStatusDone(msg.getResultCode(), EventBuilder.getGameStatus(msg));
				}
		}
		...
		try {
			...
			switch (msg.getUpdateType()) {
				...
				default:
					break;
				case 9:
					ChatEvent evt = EventBuilder.buildChatEvent(notifyData);
					for (NotifyListener listener : this.notifyListeners) {
				...
				case 12:
					...
					return;
				case 13:
					MoveEvent moveEvent = EventBuilder.buildMoveEvent(notifyData);
					for (NotifyListener listener : this.notifyListeners) {
	 ...
	 */
	public void getRoomsWithProperties(Map<String, Object> properties) {
		byte errorCode = 0;
		boolean var12 = false;
			...
			this.storageService.findDocumentsByQueryWithPaging(this.dbName, "ChatHistory", q1, max, offset,
					new App42CallBack() {
						public void onSuccess(Object response) {
							JavaWarpClient.this.onApp42Response(1, response);
						}

						public void onException(Exception ex) {
							JavaWarpClient.this.onApp42Exception(1);
						}
					});
		} else {
			for (ChatRequestListener listener : this.chatRequestListeners) {
	...
	void onApp42Response(int app42RequestCode, Object response) {
		if (app42RequestCode == 1) {
			...
			List<ChatEvent> chats = EventBuilder.buildChatHistoryList(storage.getJsonDocList());
			for (ChatRequestListener listener : this.chatRequestListeners) {
	...
	private class ConnectionWatchTask extends Timer.Task {
		...
		public void run() {
			if (this.owner.connectionState != 0) {
				byte result = 5;
				if (JavaWarpClient.this.connectionState == 4) {
					result = 9;
				}
